...
impl<'a> CanCreate<Task, CreateTaskWithUserRequest<'a>> for TaskRepository {
    async fn create(&self, item: &CreateTaskWithUserRequest<'a>) -> sqlx::Result<Task> {
        ...
        sqlx::query_as::<_, Task>(&sql)
            ...
            .bind(item.request.list_id)
            .bind(item.user_id)
            .bind(now)
            .bind(now)
            .bind(&item.request.title)
            .bind(&item.request.description)
            .bind(item.request.done)
            .fetch_one(&self.pool)
            .await
    }
}
