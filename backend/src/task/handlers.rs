...
pub async fn create_task(task: web::Json<CreateTaskRequest>, repo: web::Data<TaskRepository>, user: User) -> impl Responder {
    if let Err(err) = task.validate() {
        return ApiError::BadParam(&err.to_string()).error_response();
    }

    let result = repo.create(&task.with_user(user.id)).await;
    match result {
