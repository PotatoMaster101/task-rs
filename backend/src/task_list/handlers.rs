...
pub async fn create_task_list(list: web::Json<CreateTaskListRequest>, repo: web::Data<TaskListRepository>, user: User) -> impl Responder {
    if let Err(err) = list.validate() {
        return ApiError::BadParam(&err.to_string()).error_response();
    }

    let result = repo.create(&list.with_user(user.id)).await;
    match result {
