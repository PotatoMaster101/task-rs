use uuid::{uuid, ContextV7, Timestamp};
use crate::repo::*;
use crate::user::*;
use crate::user::requests::*;

const USER_TABLE_NAME: &str = r#""app"."user""#;
...
impl CanCreate<User, CreateUserRequest> for UserRepository {
    async fn create(&self, item: &CreateUserRequest) -> sqlx::Result<User> {
        let sql = format!(r#"INSERT INTO {} VALUES ($1, $2, $3, $4, $5, $6) RETURNING *"#, USER_TABLE_NAME);
        let now = Utc::now();
        sqlx::query_as::<_, User>(&sql)
            .bind(Uuid::new_v7(Timestamp::now(ContextV7::new())))
            .bind(uuid!(USER_ROLE_ID))
            .bind(now)
            .bind(now)
            .bind(&item.email)
            .bind(&item.auth0_id)
...
impl CanGet<User, String> for UserRepository {
    async fn get(&self, item: &String) -> sqlx::Result<User> {
        let sql = format!(r#"SELECT * FROM {} WHERE "auth0_id" = $1"#, USER_TABLE_NAME);
        sqlx::query_as::<_, User>(&sql)
            .bind(item)
            .fetch_one(&self.pool)
            .await
